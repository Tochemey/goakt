...
- **Concurrent spawns of one name leave one actor** ([#1377](https://github.com/Tochemey/goakt/issues/1377)). Two nodes spawning the same actor name at the same time could both succeed and keep two live actors, and stopping either one deleted the other's registry record. Now the first spawn wins and the others fail with `ErrActorAlreadyExists`, the error a taken name already returned, and leave nothing behind. A registry record is updated or removed only by the actor that owns it, so a stopped or rolled-back copy never overwrites or deletes the live one. During a rolling upgrade, nodes on the previous version still publish the old way, so the guarantee holds once every node is upgraded.
