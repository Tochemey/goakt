...
GoAkt's architecture (Hashicorp Memberlist for gossip, a per-node actor system, and cluster-wide pub/sub via the TopicActor) is ideally suited to host a CRDT replication layer.
...
|----------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Zero import cycles**                 | The `crdt` package contains only pure data types and config, with no dependency on `actor`, `internal`, or any GoAkt package. The `actor` package imports `crdt/` as a leaf dependency (for `crdt.Option` and CRDT types). The Replicator actor lives in `actor/` alongside the other system actors. `crdt/` never imports `actor/`. No cycles. |
...
| **GC-friendly**                        | CRDT types keep allocations predictable: capacity-hinted maps, no closures in merge paths, and periodic compaction of causal metadata. See [Section 14](#14-gc-and-memory-efficiency).                                                                                                                                                          |
...
└── actor/
    ...
    └── reserved.go                ← replicatorType reserved name ("GoAktReplicator")
```
...
Types that carry causal metadata (ORSet, ORMap) also implement the optional `Compactable` interface (`CompactData() ReplicatedData`). The Replicator calls `CompactData` during periodic prune cycles to reclaim memory.
...
| **CRDT Messages**    | `crdt/` (public)    | Plain Go structs (`Update`, `Get`, `Subscribe`, `Unsubscribe`, `Delete`, `Changed`) that user actors send to their local Replicator via Tell/Ask.                                                                    |
...
4. It is spawned as a **child of the system guardian** with `asSystem()`, `WithLongLived()`, and a one-for-one supervisor that restarts on any failure.
...
- `ActorSystem.Start()` calls `spawnReplicator(ctx)` after the startup sequence (cluster join, TopicActor, remoting) completes.
...
5. Encodes the delta as protobuf and publishes to the shared CRDT topic. If cross-DC replication is enabled, the encoded delta is also appended to the pending buffer.
...
```go
// Push the delta to a majority of peers before the response is returned.
...
func (a *MyActor) Receive(ctx *actor.ReceiveContext) {
    ...
    case *actor.PostStart:
        replicator := ctx.ActorSystem().Replicator()
        if replicator != nil {
            ctx.Tell(replicator, &crdt.Subscribe{Key: activeSessions})
        }
    case *crdt.Changed:
        if msg.Data != nil {
            a.sessions = msg.Data.(*crdt.ORSet).Elements()
        }
    }
}
```

`Subscribe` must be sent from inside an actor (for example with `ctx.Tell`) so that the Replicator sees a sender PID to register and death-watch. A sender-less `Subscribe` (for example via the package-level `actor.Tell`) is silently ignored.
...
Deletion removes the key from the local store and publishes a `CRDTTombstone` to the shared topic via TopicActor. Peer Replicators that receive the tombstone remove the key from their stores. `Delete` also accepts an optional `WriteTo` level; the tombstone is then pushed directly to the selected peers via RemoteTell before the TopicActor publish. Tombstones are retained for a configurable TTL (`WithTombstoneTTL`, default 24h) and pruned periodically (`WithPruneInterval`, default 5m). While a tombstone is active, updates and deltas for that key are rejected to prevent resurrection.
...
- **`WriteTo: Majority` / `All`**: the Replicator sends the delta directly to peer Replicators via synchronous RemoteTell calls before responding. Peers that fail lookup or delivery are logged and skipped; they do not fail the write. The delta is *also* published via TopicActor so that peers not in the direct set receive it.
- **`ReadFrom: Majority` / `All`**: the Replicator sends RemoteAsk (with `CoordinationTimeout`, default 5s) to peer Replicators, collects their local values, merges them with the local value, stores the merged result locally, and returns it.

### Consistency Guarantee

Coordination is best-effort: unreachable peers are skipped rather than failing the operation, so `WriteTo` and `ReadFrom` narrow the staleness window but do not guarantee quorum intersection. With or without coordination, the system is **eventually consistent** (strong eventual consistency: replicas that have applied the same updates hold the same state), with convergence bounded by TopicActor's delivery latency and the anti-entropy interval.
...
ActorSystem.Start() [on each node]
    ...
    │
    ├── Start() calls spawnReplicator(ctx) after the startup sequence completes
    │       │
    │       ├── Skips when clusterConfig.crdtConfig == nil
    │       ├── Checks role filter (skip if node's roles don't match)
...
message ORSetData {
    ...
    message ORSetEntry { bytes element = 1; repeated ORSetDot dots = 2; }
    repeated ORSetEntry entries = 1;
    map<string, uint64> clock = 2;
}

message ORMapData {
    message ORMapEntry { bytes key = 1; CRDTData value = 2; }
...
message MVRegisterData {
    message MVRegisterEntry { bytes value = 1; string node_id = 2; uint64 counter = 3; }
...
message CRDTReadResponse {
    CRDTKey key = 1;
    CRDTData data = 2;  // nil if the key is not found
    string from_node = 3;
}
...
| Rule                           | Rationale                                                                                                                                                                                                         |
|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Copy-on-write values**       | All CRDT types use pointer receivers but behave as immutable values: every mutation (`Increment`, `Add`, `Set`, ...) returns a new value. No hidden shared state.                                                 |
| **Pre-sized maps**             | `make(map[K]V)` calls use a capacity hint wherever the target size is known (`Clone`, `State`, `Delta`, merge paths).                                                                                             |
| **Snapshot accessors**         | `ORSet.Elements()` (a fresh `[]any`) and `ORMap.Entries()` (a copied `map[any]ReplicatedData`) return snapshots, so internal state never escapes to callers.                                                      |
...
| **Concrete counter state**     | Counters keep concrete `map[string]uint64` state. Set, map, and register elements are stored as `any` by design (they must be runtime-comparable); the `ReplicatedData` interface appears only at API boundaries. |

### Per-Type Allocation Profile

| Type            | Internal State                                                | Merge Allocation                                                                                   |
|-----------------|---------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| **GCounter**    | `state` and `delta` maps (`map[string]uint64`)                | Merge clones the local counter (two capacity-hinted maps), then applies the per-node max in place. |
| **PNCounter**   | Two `*GCounter` values (increments, decrements)               | Same as GCounter x 2.                                                                              |
| **LWWRegister** | `value any`, `timestamp int64`, `nodeID string`, `dirty bool` | One small struct allocation per merge; the winning value is referenced, not copied.                |
| **ORSet**       | `map[any][]dot`, `map[string]uint64` clock, delta maps        | Dot slices grow with concurrent adds. `Compact` (run on the prune tick) reclaims dominated dots.   |
| **ORMap**       | `*ORSet` for keys + `map[any]ReplicatedData` for values       | Per-key CRDT merge allocation + key-set OR-Set merge.                                              |
| **Flag**        | `enabled bool`, `dirty bool`                                  | One small struct allocation per merge.                                                             |
...
Merges are copy-on-write (they clone local state before combining), so merge allocations scale with state size rather than being zero. `crdt/benchmark_test.go` tracks allocations for the merge, increment, clone, and delta paths of every type via `ReportAllocs`.
