...
When clustering is enabled, the endpoint publishes through `PutActor` like every other actor: the first write claims the name under a cluster-wide per-name lock and fails with `ErrActorAlreadyExists` when another incarnation owns it, and later writes by the owning incarnation, such as a restart, update the record.
