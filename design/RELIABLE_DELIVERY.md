...
When clustering is enabled, the endpoint publishes through `PutActor` like every other actor: the first write claims the name with a conditional write and fails with `ErrActorAlreadyExists` when another incarnation owns it, and later writes by the owning incarnation, such as a restart, update the record.
...
3. Before the respawn, `recreateActorFromWire` releases the departed incarnation's companion record, deriving the identity from the record's role and incarnation and through `releaseDepartedEntry`, whose removal is fenced by the incarnation the record carried when it was read. The release can never touch the fresh controller. Reconstruction failure, for example a queue type not registered on the survivor, restores the departed endpoint record for a later relocation retry.
...
6. Non-relocatable reliable endpoints get their registry records withdrawn on node loss instead of leaking. This is required, not hygiene: a registry write claims the actor name, so a leaked record would block the endpoint name cluster-wide.
