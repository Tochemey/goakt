...
- `ActorRef` → `*PID`
- `ActorOf` returning `(addr, pid, err)` → `ActorOf` returning `(*PID, error)`
- `Actors` / `ActorRefs` → `Actors`
- `LocalActor` → `ActorOf`
- `RemoteActor` → `ActorOf`
- `RemoteSchedule*` methods → `Schedule*` with remote PID from `ActorOf`
- `GetPartition` → `Partition`
- `pid.Address` → `pid.Path`
- `ctx.SenderAddress` → `ctx.Sender`
- `ctx.ReceiverAddress` → `ctx.Self`
- `remote.Remoting` / `remote.Client` → Actor system APIs; configure via `WithRemote`
- `WithRemoting` → `WithRemote`
...
- **PID.Tell** / **PID.Ask**: Old: `message proto.Message`. New: `message any`.
- **PipeTo** / **PipeToName**: task functions likewise deliver `any` instead of `proto.Message`.
- **Schedule** / **ScheduleOnce** / **ScheduleWithCron**: Old: `message proto.Message`. New: `message any`.
- **AskGrain** / **TellGrain**: Old: `message proto.Message`. New: `message any`.
...
- **Actor found locally**: Old: `(addr, pid, nil)`. New: `(pid, nil)`: local PID.
- **Actor found in cluster**: Old: `(addr, nil, nil)`. New: `(pid, nil)`: remote PID.
- **Not found**: Old: `(nil, nil, err)`. New: `(nil, err)`.
...
- `testkit.Probe.SenderAddress` → `Sender` then `pid.Path` or `pid.IsRemote`
- `ctx.SenderAddress` → `ctx.Sender` (guard for nil sender)
- `ctx.ReceiverAddress` → `ctx.Self`
...
- Optional TLS
- Compression: `NoCompression` (default), `GzipCompression`, `ZstdCompression`, `BrotliCompression`
...
- **WithHostAndPort**: Target remote node.
- **WithRelocationDisabled**: No relocation on host failure.
- **WithDependencies**: Injected on remote node.
- **WithStashing**: Enable stashing.
- **WithPassivationStrategy**: Passivation behavior.
- **WithReentrancy**: Async request policy.
- **WithSupervisor**: Failure handling.
- **WithRole**: Require remote node role.

---

## Migration Checklist

- Update import path: `goakt/v3` → `goakt/v4`
- Replace `proto.Message` with `any` in handlers and call sites
- Replace `goaktpb.*` with `actor.*`; use constructors for structs
- Replace `ActorRef` with `*PID`
- Replace `LocalActor` / `RemoteActor` with `ActorOf(ctx, name)`
- Replace `pid.Address()` with `pid.Path()`
- Replace `ctx.SenderAddress()` / `ctx.ReceiverAddress()` with `ctx.Sender().Path()` / `ctx.Self().Path()`
- Replace `GetPartition` with `Partition`
- Replace `WithRemoting` with `WithRemote(config)`
- Remove `address` package imports; use `Path` interface
- Update custom `Logger` implementations with new methods
- Update tests: `Probe.SenderAddress()` → `Sender()` + `Path()`
- Remove `RemoteSchedule*` usage; use `Schedule*` with `ActorOf` PID
