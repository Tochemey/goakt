...
func TestEventSourcedActor(t *testing.T) {
	...
	t.Run("with state recovery from event store", func(t *testing.T) {
		...
		eventStore := postgres2.NewJournalStore(config)
		...
		assert.NoError(t, actorSystem.StopActor(ctx, behavior.Kind()))
		// wait a while
		time.Sleep(time.Second)

		// restart the actor
		pid, err = actorSystem.RestartActor(ctx, behavior.Kind())
		assert.NoError(t, err)

		// fetch the current state
		command = &pb.GetStateCommand{}
		reply, err = actors.SendSync(ctx, pid, command, time.Second)
		require.NoError(t, err)
		require.NotNil(t, reply)
		require.IsType(t, new(pb.CommandReply), reply)
