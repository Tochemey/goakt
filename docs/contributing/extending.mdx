...
Create a sub-package under `discovery/`. Pass the provider to `ClusterConfig` when creating the actor system. See
`discovery/selfmanaged` for a built-in zero-config provider using UDP broadcast.
...
`WithExtensions(ext)` when creating the actor system. Actors access via `ctx.Extension(id)` and type assertion. See
[Extensions and Dependencies](/advanced/extensions-and-dependencies) for usage.

## Dependency (per-actor)

```go Dependency (per-actor)
package extension
