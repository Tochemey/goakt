...
- **make unit-test**: Run the test suite the way CI does, with the race detector, as parallel shards in pristine containers that need only Docker and the checkout. `make unit-test SHARD=core` runs one shard; the shards mirror the CI matrix and are defined in `scripts/test-shard.sh`. Per-shard logs land in `.unit-test/` and coverage in `coverage-<shard>.out`.
