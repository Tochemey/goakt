...
| ----------------------- | ---------------------- | ------------------------------------------------- |
| Worker pool size        | `max`                  | `dispatcherWorkerCount` in `actor/dispatcher.go`  |
...
- [Context Pools](/architecture/context-pools): sharded `ReceiveContext` / `GrainContext` recycling on the send and dequeue path
