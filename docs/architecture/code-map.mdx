...
Pure CRDT data types and replication configuration—**no** import of `actor`. User and Replicator code interact via `crdt` messages and `ReplicatedData` values. See [Distributed data](/advanced/distributed-data) and the repository `arch/CRDT_ADR.md`.
...
Reactive pipelines (`Source`, `Flow`, `Sink` → `RunnableGraph`) materialized onto an `ActorSystem` with backpressure. See [Streams](/advanced/streams) and repository `arch/REACTIVE_STREAM_ADR.md`.
...
- [Architecture Document](https://github.com/tochemey/goakt/blob/main/arch/ARCHITECTURE.md) in `arch/` for full details, data flows, and design decisions
