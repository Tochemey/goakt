...
| `context_pool.go`                              | Sharded lock-free pool for `ReceiveContext` objects. See [Context Pools](/architecture/context-pools). |
...
| `grain_context_pool.go`       | Sharded lock-free pool for `GrainContext` and Tell ack channels. See [Grain sibling](/architecture/context-pools#grain-sibling). |
...
- [Context Pools](/architecture/context-pools): sharded `ReceiveContext` / `GrainContext` recycling on the messaging hot path
