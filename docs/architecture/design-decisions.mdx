...
Every `Tell` and `Ask` needs a `ReceiveContext`. A single process-wide buffered channel serialized every get and put on one lock; pairwise `Tell` spent most of its CPU inside `chansend`/`chanrecv`, so aggregate throughput collapsed to the single-actor rate. `sync.Pool` is per-P and may drop everything on GC, which is the opposite of predictable reuse.

The pool is sharded by actor: each PID is assigned a home shard at spawn, and contexts for that actor are taken from and returned to that shard. Shard count tracks `GOMAXPROCS` so concurrently active actors *spread* across rings; many actors still share a shard. Capacity is a memory cap, not a throughput cap: empty shards allocate, full shards drop for GC. Grains use a separate copy of the same ring (`grain_context_pool.go`) plus a sharded Tell-ack channel pool. See [Context Pools](/architecture/context-pools) for the actor ring and [Grain sibling](/architecture/context-pools#grain-sibling) for the delta.
