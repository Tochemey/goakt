...
Every registry-derived recovery is announced by a `RelocationDerived` event on the event stream, listing the actor names and grain IDs the derivation found. The event fires even when the derived set is empty, which on a crash signals suspected record loss rather than a benign no-op. Subscribers holding an external record of placements can diff it against the listed items to detect and reconcile silent losses.
