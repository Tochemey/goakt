...
The cluster-wide claim's TTL is scaled to the cron schedule's own period (clamped between one minute and one hour): the TTL must outlive the worst-case delivery spread between nodes racing for the same tick, since a node that claims after the TTL of a stale entry has already elapsed is otherwise indistinguishable from a node handling the next tick. `CancelSchedule` and actor system shutdown proactively remove the most recently attempted claim key; the TTL is only the backstop for anything a crash left behind.
...
<Note>
Cluster-wide single fire requires the builtin cluster engine, which is what `WithCluster` configures. A custom `Cluster` implementation that cannot atomically arbitrate the claim causes `ScheduleWithCron` to return `ErrSingleFireUnsupported` at registration time rather than silently duplicate or drop deliveries.
</Note>
...
| `ErrSingleFireUnsupported`      | `ScheduleWithCron` in cluster mode with a `Cluster` implementation that cannot atomically arbitrate the claim |
