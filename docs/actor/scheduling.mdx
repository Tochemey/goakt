...
`ListSchedules()` returns a read-only snapshot of every schedule the scheduler currently knows about. Use it to build admin tooling, dashboards, or to answer "what will fire next, and when?" during operational debugging.

```go
type ScheduleInfo struct {
	Reference    string        // the schedule reference (user-supplied or auto-generated)
	TriggerKind  TriggerKind   // "once", "interval", or "cron"
	Expression   string        // cron expression; only set for TriggerKindCron
	Interval     time.Duration // interval, or the original delay for TriggerKindOnce; zero for TriggerKindCron
	NextFireTime time.Time     // next delivery time, as reported by the scheduler
	Address      string        // target actor address
}
```

```go
for _, info := range system.ListSchedules() {
	fmt.Printf("%s (%s) -> %s, next fire at %s\n",
		info.Reference, info.TriggerKind, info.Address, info.NextFireTime)
}
```
