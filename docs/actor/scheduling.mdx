...
The cluster-wide claim's TTL is scaled to the cron schedule's own period, clamped between one minute and 24 hours (the same window within which the scheduler still executes a delayed tick). The TTL must outlive the worst-case delivery lag between nodes handling the same tick: a node that claims after a stale entry has already expired would win the same tick again and deliver a duplicate. Claim entries are reclaimed by their TTL alone; canceling a schedule or stopping a node never deletes them, since an entry may be the winning claim other nodes are still arbitrating against.
