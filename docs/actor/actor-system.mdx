...
| `SpawnOn`            | Create actor on a cluster node (see [Placement](/clustering/placement)) or in another data center (`WithDataCenter`).        |
...
| `SpawnSingleton`     | Cluster singleton; one instance per name across the cluster, hosted on the oldest node. See [Singletons](/actor/singletons). |

See [Actor Lifecycle](/actor/lifecycle), [Routers](/actor/routers), [Singletons](/actor/singletons), [Clustering](/clustering/overview).
...
| Method   | Purpose                                                                                                                                                                                    |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Inject` | Register dependency types with the actor system after startup. Enables restoration during cluster topology changes and when creating actors on remote hosts. Must be called after `Start`. |
...
| `WithExtensions`              | System-wide extensions. See [Extensions and Dependencies](/advanced/extensions-and-dependencies).                                                                      |
...
| **Advanced**                  |                                                                                                                                                                        |
| `WithThroughputBudget`        | Messages an actor may process per scheduler turn (default: 32). See [Dispatcher Pool](/architecture/dispatcher-pool#tuning-the-throughput-budget) for tuning profiles. |
| `WithPartitionHasher`         | Deprecated: use `ClusterConfig.WithPartitionHasher`. See [Partition Hashing](/clustering/partition-hashing).                                                           |
