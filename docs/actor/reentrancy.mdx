...
| `WithMaxInFlight` | Cap the number of outstanding async requests. When reached, `Request`/`RequestName` return `ErrReentrancyInFlightLimit`. Use `n <= 0` for no limit. |

Without a reentrancy policy, calls to `Request`, `RequestName`, or `RequestGrain` fail with `ErrReentrancyDisabled`.

## Runtime toggle

An actor spawned without reentrancy can enable it from inside a handler when the capability is only needed for a particular case, and turn it off again afterward:
...
| `ctx.RequestGrain` | Send an async request to a grain, which activates on demand like any other grain send. Returns a `RequestCall` or `nil`. |
...
| Method   | Purpose                                                                                                                                                                                                                            |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Then`   | Register a continuation `func` invoked when the request completes (success, error, timeout, or cancel). Only the first call is honored. If the request already completed, the callback runs immediately in the caller's goroutine. |
...
| Option               | Purpose                                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------------------ |
| `WithReentrancyMode` | Override the actor-level mode for this request only. Cannot enable if actor has reentrancy off.        |
| `WithRequestTimeout` | Set a per-request timeout. On expiry, the request completes with `ErrRequestTimeout`. `<= 0` disables. |
...
| Error                        | When                                                                                                                                   |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `ErrReentrancyDisabled`      | The actor has no reentrancy policy (neither `WithReentrancy` at spawn nor a later `EnableReentrancy`), or the effective mode is `Off`. |
...
- [Grain Reentrancy](/grains/reentrancy) for the grain-side API: `RequestGrain`/`RequestActor` from grains, `DeferResponse`, and paused-consumption stash semantics.
