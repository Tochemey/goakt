...
| `WithConsistentHashRouter`       | Use consistent-hash routing. `extractor` is a `MessageRoutingKeyExtractor` (`func(msg any) string`). Messages with the same key go to the same routee. Returns empty string to fall back to random. |
| `WithConsistentHashVirtualNodes` | Virtual nodes per routee for the hash ring (default 150). Only with `WithConsistentHashRouter`.                                                                                                     |
...
| `AsTailChopping`                 | Use tail-chopping. `within` is the global deadline; `interval` is the delay between successive attempts. Requires `interval` > 0 and `within` > 0; typically `interval` < `within`.                 |
