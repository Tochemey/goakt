...
| `RelocationDerived` | A crashed node's relocation set was reconstructed from the cluster registry, best-effort (cluster mode only) |
...
- `RelocationFailed` is published by the relocator when a departed node's actors or grains cannot be re-deployed. Relocation is not retried; subscribe to this event to drive your own recovery (for example, re-spawning the affected actors or alerting).
- `RelocationDerived` is published by the leader when a node crashes without leaving a graceful-shutdown snapshot and its relocation set is reconstructed from the replicated cluster registry. The derived set is best-effort: records lost with the crashed node's partitions cannot be listed, so an empty or unexpectedly small set signals suspected record loss. See the [relocation](/actor/relocation) page for the replication settings that govern completeness.
