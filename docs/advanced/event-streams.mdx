...
| `LeaderChanged`     | The local node's coordinator status changed (cluster mode only)              |
...
for msg := range subscriber.Iterator() {
    ...
    case *actor.LeaderChanged:
        log.Printf("leadership changed: address=%s isLeader=%t", e.Address(), e.IsLeader())
...
- Lifecycle events (ActorStarted, ActorStopped, etc.) are published by the PID and death-watch logic. Cluster events (NodeJoined, NodeLeft) are published by the cluster events loop when membership changes. `LeaderChanged` is published from the same loop whenever a topology event flips the local node's coordinator status; the initial coordinator election on startup is not published, only subsequent transitions.
