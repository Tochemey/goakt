...
| Option             | Purpose                                                                                            |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| `WithRemoteConfig` | Set remoting config (compression, serializers, timeouts, TLS). Required for correct serialization. |
...
| Method      | Purpose                                                                                                                                                            |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Tell`      | Fire-and-forget to an actor. `message` must be `proto.Message`. Returns `ErrActorNotFound` if the actor is not found.                                              |
| `Ask`       | Request-response to an actor. Blocks until reply or timeout. `message` accepts `any` (must be serializable). Returns `ErrActorNotFound` if the actor is not found. |
| `TellGrain` | Fire-and-forget to a grain. `message` must be `proto.Message`. The grain kind must be registered on the cluster via `RegisterGrainKind`.                           |
...
| Method   | Purpose                                                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Exists` | Check if an actor is registered in the cluster. Returns `(false, nil)` when the actor does not exist; returns an error only on lookup failure (e.g. network). |
...
| `WithRefresh`          | Periodically refresh node weights by querying each node's load. Used by `LeastLoadStrategy` to prefer the least-loaded node. |
