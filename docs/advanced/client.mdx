...
| `WithTLS(info)`            | Connect to a TLS-enabled cluster. Only `ClientConfig` of the `tls.Info` is used; it must chain to the cluster's root CA. See [TLS](/advanced/remoting#tls). |

### Connecting to a TLS-enabled cluster

Pass the same `tls.Info` used by the cluster nodes (or any `tls.Info` whose `ClientConfig` trusts the cluster's CA
and, when the cluster enforces mutual TLS, presents a certificate that chains to it):

```go
nodes := []*client.Node{
    client.NewNode("127.0.0.1:8080",
        client.WithRemoteConfig(remote.NewConfig("127.0.0.1", 0)),
        client.WithTLS(&tls.Info{ClientConfig: clientTLSConfig}),
    ),
}
```

The client only dials, so the `ServerConfig` field is ignored. A client without `WithTLS` cannot talk to a
