...
- **Cluster propagation lag.** When the producer side spawns the endpoint and immediately ships the ref to a consumer that resolves it before the cluster's actor registry has propagated, the bridge retries with jittered exponential backoff for up to 30s. The propagation pipeline is async — `putActorOnCluster` queues, a single goroutine drains it via `cluster.PutActor`, and olric then replicates to the consumer node — so under heavy load (many actor systems sharing a host on CI, slow olric peers, large cluster sizes) it can still time out. The bridge surfaces a `"resolve source ref"` / `"resolve sink ref"` error rather than hanging; plan for it in the receiving side's error handling, and prefer creating the ref well before publishing it whenever possible.
