...
- **Generic type constraints** — `ORSet[T]` requires `T` to be `comparable`. `LWWRegister[T]`, `MVRegister[T]`, and `ORMap[K, V]` require values to be serializable by the codec layer. The built-in codec currently supports `ORSet[string]`, `MVRegister[string]`, `ORMap[string, *GCounter]`, `GCounter`, `PNCounter`, and `Flag`.
