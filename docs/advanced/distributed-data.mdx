...
- **Value serialization** — CRDT values are serialized using a composite Proto+CBOR serializer. Go primitives (`string`, `int`, `bool`, `float64`, etc.) and `proto.Message` types work out of the box. Custom struct types must be registered via `remote.WithSerializers(new(MyType), remote.NewCBORSerializer())`.
- **No partial updates** — The `Modify` function in `Update` receives the full current value and must return the full updated value. There is no patch or field-level delta mechanism.
