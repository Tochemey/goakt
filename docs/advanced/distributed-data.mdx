...
- **Generic type constraints** — `ORSet[T]` requires `T` to be `comparable`. `LWWRegister[T]`, `MVRegister[T]`, and `ORMap[K, V]` require values to be serializable by the codec layer. The built-in codec currently supports `GCounter`, `PNCounter`, `Flag`, `LWWRegister[string]`, `MVRegister[string]`, `ORSet[string]`, and `ORMap[string, *GCounter]`.
