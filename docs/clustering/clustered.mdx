...
carrying the new leader's address. Compare it with `ActorSystem.PeersAddress()` to know whether the local node is
the one that took over.

```go
for msg := range subscriber.Iterator() {
    if leaderChanged, ok := msg.Payload().(*actor.LeaderChanged); ok {
        becameLeader := leaderChanged.Address() == system.PeersAddress()
        log.Printf("cluster leader is now %s (self=%t)", leaderChanged.Address(), becameLeader)
    }
}
```
