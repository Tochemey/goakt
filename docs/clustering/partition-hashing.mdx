...
| ----------------------------- | --------------------------------------------------- | -------------------------------- |
| Cluster registry partitioning | `WithPartitionHasher(hasher)` (actor system option) | Cluster mode only                |
...
system, err := actor.NewActorSystem("accounts",
    ...
    actor.WithPartitionHasher(myHasher{}),
)
```
...
In standalone mode there is no distributed registry and no partitions, so `WithPartitionHasher` has no effect. It is safe to set (useful when the same configuration code path serves both modes), but the hasher is never invoked.
