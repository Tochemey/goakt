...
Both options reject finite passivation: reliable endpoints are long-lived. `AsWorkPullingProducer` rejects `WithChunking` and the point-to-point `WithDurableQueue`; durability uses `WithDurableWorkQueue` because per-message completion across workers cannot be expressed by a cumulative watermark.
...
Without a durable work queue, a producer-side restart loses jobs already accepted into the controller but not yet confirmed by a worker. To survive producer crashes and relocation, attach a `DurableWorkQueue`:

```go
dispatcher, err := system.Spawn(ctx, "job-dispatcher", &JobDispatcher{},
    actor.AsWorkPullingProducer(
        actor.WithDurableWorkQueue(queue),
        actor.WithQueueRetry(3, 100*time.Millisecond)))
```

The contract keeps Store and Accept on the same first-write-wins, epoch-fenced append model as point-to-point, but confirmation is per `MessageID`:
...
`WorkQueueState` carries the append cursor and every accepted, not-yet-confirmed message in store order. Holes are normal: worker A may confirm one job while worker B still holds an earlier store sequence. On reload, those unconfirmed jobs re-enter the pending pool and re-dispatch to whichever workers are registered. `ConfirmMessage` is idempotent for an already confirmed `MessageID` and returns `ErrQueueConflict` for an unknown one. Register the queue type on every eligible node when the producer may relocate, exactly as for point-to-point durable queues.

<Warning>
A durable work-pulling flow still performs `Store` plus `Accept` before the next credit, plus a `ConfirmMessage` per completed job. Backend latency bounds per-flow throughput; scale with independent producer flows when needed.
</Warning>
...
| `WithDurableWorkQueue`     | Producer | off     | Persist accepted jobs; reload re-dispatches unconfirmed work after a crash       |
| `WithQueueRetry`           | Producer | 3 / 100ms | Retry policy for durable work-queue operations                                |
