...
Eager relocation is the exception to call-site reactivation: the runtime retains the serialized activation role and
only recreates the grain on a surviving node that advertises it. If no eligible node survives, relocation reports the
grain through `RelocationFailed` instead of activating it on an unsuitable node.
