...
- **Actor**: API: `Spawn`. Placement decided: At spawn: the calling node, or an explicit node via `WithHostAndPort`.
  Default strategy: Local.
- **Actor**: API: `SpawnOn`. Placement decided: At spawn: node selected by strategy and role. Default strategy:
  `RoundRobin`.
- **Singleton actor**: API: `SpawnSingleton`. Placement decided: At spawn: coordinator, or oldest node with the role.
  Default strategy: Leader.
- **Grain**: API: `GrainOf` / first message. Placement decided: At activation: node selected by strategy and role.
  Default strategy: `LocalActivation`.
...
```go Spawning on an explicit node
...
- The actor kind must be registered on the target node, through `ClusterConfig.WithKinds` in cluster mode or
  `ActorSystem.Register` in remoting-only setups; otherwise the target rejects the request with `ErrTypeNotRegistered`.
...
- **RoundRobin** (default): Selection: Cycles through candidate nodes. Notes: Uses a cluster-wide shared counter, so the
  rotation is even across callers. Actors and grains keep separate counters.
- **Random**: Selection: Uniform random candidate. Notes: Stateless and cheap; distribution can be uneven over small
  samples.
- **Local**: Selection: Always the calling node. Notes: Use when locality matters (node-local caches, files, devices).
- **LeastLoad**: Selection: Candidate reporting the smallest load. Notes: Queries every candidate's load metric over
  remoting before spawning; placement cost grows with cluster size.

```go Strategy-based placement with SpawnOn
...
An actor name identifies one actor across the cluster. Spawning a name that is already taken returns
`ErrActorAlreadyExists`. When several nodes spawn the same name at the same time, one spawn succeeds and the others
return `ErrActorAlreadyExists` and leave nothing behind. Stopping an actor never removes another actor's registration.

<Note>
  Strategies see the topology at spawn time. `RoundRobin` gives an even spread over a stable membership, but actors do
  not move when nodes join later; redistribution only happens through [relocation](/actor/relocation) when a node
  leaves.
</Note>
...
Nodes advertise roles through `ClusterConfig.WithRoles`. Roles let you dedicate node pools to workload classes ("api",
"entity", "projection") and scale them independently.

```go Role-constrained placement
...
- If no node in the cluster advertises the role, `SpawnOn` fails with an error instead of falling back to an unsuitable
  node.
...
```go Cross data center placement
...
during that redistribution. Use `WithRelocationDisabled` to pin an actor to the node it was placed on, accepting that it
is lost if that node goes down; its name is released so it can be spawned again. See [Relocation](/actor/relocation).
...
registry with an atomic put-if-absent, so two nodes racing to activate the same identity converge on a single owner; the
loser routes to the winner. See the [activation guarantee](/grains/overview#activation-guarantee).

### Activation strategies

- **LocalActivation** (default): Selection: The calling node. Notes: The grain lives where it is first used.
- **RoundRobinActivation**: Selection: Cycles through candidate nodes. Notes: Cluster-wide shared counter, independent
  from the actor counter.
- **RandomActivation**: Selection: Uniform random candidate. Notes: Stateless spreading.
- **LeastLoadActivation**: Selection: Candidate reporting the smallest load. Notes: Queries every candidate's load
  metric over remoting; activation cost grows with cluster size.

```go Activation strategies
...
`WithActivationRole` restricts activation to nodes advertising the role, with the same semantics as actors: the strategy
applies among matching nodes, activation fails with an error when no node advertises the role, and the option is ignored
without clustering.

```go Role-constrained activation
...
Grain options, including the activation strategy and role, take effect on the activation performed by the `GrainOf` call
that carries them. A grain that passivated and is later reactivated by a bare `TellGrain` or `AskGrain` on a stored
identity comes back with the default configuration. When placement must be enforced on every activation, resolve the
identity through `GrainOf` with your options before messaging.
...
that can host the grain must know the kind: `GrainOf` auto-registers it on the calling node, and other nodes register it
at startup with `RegisterGrainKind` or `ClusterConfig.WithGrains`. Activation on a node that does not know the kind
fails.
...
- **Lazy relocation (default)**: the departed node's grain directory entries are cleaned and each grain reactivates on a
  surviving node the next time it is addressed. The next activation runs the placement flow again.
- **`WithGrainEagerRelocation`**: the grain is reactivated immediately on a surviving node when its host departs. Use it
  for grains that must stay warm, such as those driving timers or background work.
...
- Use `LeastLoad` / `LeastLoadActivation` for heavyweight, long-lived workloads where one extra round of metric queries
  at placement time is cheap compared to running on a busy node.
