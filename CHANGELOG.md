...
#### Remoting stack: fewer allocations per message and no global-lock lookups on the hot path

The proto-over-TCP remoting path (client and server) was profiled end to end and reworked to remove per-message allocations and shared-lock contention. Protobuf message types are now resolved through a lock-free cache in front of `protoregistry.GlobalTypes`, including bounded negative caching so traffic that can never resolve (for example payloads owned by a custom serializer) stops paying the registry's global mutex on every message. Outbound payloads on the synchronous ask paths serialize into pooled frame buffers, the frame pool no longer allocates a pointer box per recycle, and marshaling reuses cached proto sizes. Each connection's read side goes through a pooled 32 KiB buffered reader that travels with the connection, so small frames coalesce into one kernel read without stranding bytes between pooled exchanges. On the server, parsed sender addresses are cached (bounded at 8192 entries with a wholesale flush), every handler validates the request host against one precomputed `host:port` string instead of rebuilding it per request, and the receiver-address lookup takes the canonical wire string directly on the happy path. The serializer dispatch layer routes frames that resolve in the proto registry straight to the proto serializer instead of trial-and-error across registered serializers. The default idle connection pool per endpoint was raised from 8 to 32 and is now exported as `remote.DefaultMaxIdleConns`; the trade is up to 32 idle sockets (file descriptors) retained per remote endpoint at peak, still reclaimed by the pool's 30-second idle timeout.

Measured on Apple M1 (go1.26): a serial remote ask dropped from 43 to 40 allocations per operation, the concurrent remote ask improved by about 7% in time per operation, and ingress sender-address handling improved by 28% in time and 33% in allocations. Bytes per ask increased by roughly 18% because each ask now enforces its timeout with a real context deadline; that is part of the liveness fix described below and is a deliberate trade.

### 🐛 Fixes

#### Cross-node grain activation no longer fails identity validation

Any grain activation that had to happen on another node failed with `must contain only word characters (i.e. [a-zA-Z0-9] plus non-leading '-' or '_')`, even when the grain name was perfectly valid ([#1227](https://github.com/Tochemey/goakt/issues/1227)). This affected remote placement (for example `RoundRobinActivation` selecting a remote peer) as well as activating a grain whose registry owner lives on another node. The cause was in `sendRemoteActivateGrain`: the wire request's `Name` field was populated with the full `kind/name` identity string instead of the bare name, so the remote client rebuilt the identity as `kind/kind/name` and the receiving node rejected it during validation. The request now carries the bare grain name, matching the ask path, and the receiving node reconstructs the identity correctly as `kind/name`. A runnable two-node reproduction using the NATS discovery provider lives in `playground/issue-1227`.

#### Concurrent remote asks could hang forever

Under sustained concurrent remote asks, a caller could block forever waiting for a response that the server had silently dropped. The frame decoder returns the message type name as a zero-copy view into a pooled frame buffer, and the server returned that buffer to the pool before the handler lookup read the name; a concurrent reuse of the buffer turned the name to garbage, the lookup missed, and the request was skipped without a reply. The client never derived a deadline from the ask timeout (the timeout was only shipped to the server), so the lost response blocked the caller indefinitely. Three changes close this: the handler lookup now happens before the frame buffer is recycled; a decodable frame with no registered handler now closes the connection so mismatched peers fail fast instead of losing requests silently (pooled clients redial transparently); and `RemoteAsk`, `RemoteBatchAsk`, and `RemoteAskGrain` now bound the call with a client-side deadline derived from the ask timeout, so a lost response fails with a timeout instead of hanging. In a mixed-version cluster the close is visible as a connection reset when a newer node sends a request type an older node does not know, where previously the request was dropped silently and an ask would hang. The fix was validated with 3.6 million operations under the race detector with no reports and no hangs.

#### Remote deadline propagation is clock-skew safe and enforced consistently

Remote call metadata used to carry the client's deadline as an absolute timestamp, which coupled server-side enforcement to clock synchronization between nodes: a receiving node whose clock ran ahead of the sender by more than the timeout would fail every ask with `DeadlineExceeded`. The wire format now carries the remaining time instead (the approach gRPC uses), and the receiver derives the absolute deadline on its own clock, so enforcement is independent of cross-node clock skew and a deadline that expired in transit stays expired. Enforcement is also consistent across handlers now: every synchronous remote handler that performs cancellable work (lookup, spawn, spawn child, stop, respawn, metric, grain activation, and both ask paths) bounds its work by the propagated deadline and releases the timer on return. Fire-and-forget handlers deliberately do not create one because their context outlives the handler, and the purely in-memory handlers (watch, unwatch, reinstate) skip it because they perform no work a deadline could cancel.

#### Hardening from the remoting performance review

Three smaller defects found while reviewing the performance work are fixed. `bufferedConn.Close` no longer returns its pooled reader while a concurrent `Read` could still hold it; the connection closes first (unblocking any in-flight read), the reader is reclaimed under a lock, and reads after close return `net.ErrClosed`. Actor system shutdown now clears the sender-address cache along with the other per-run state. Host validation compares the same `net.JoinHostPort` format on both sides everywhere, fixing a mismatch that broke validation for IPv6 hosts, and a parseable but non-canonical receiver address (for example a zero-padded port) resolves again via a canonical retry.
