...
- **Point-to-point reliable delivery** ([#1296](https://github.com/Tochemey/goakt/issues/1296), [#1300](https://github.com/Tochemey/goakt/issues/1300)). One producer actor hands messages to one consumer actor with confirmed, ordered, flow-controlled delivery, locally or across a cluster. Default `Tell` is unchanged. A flow is enabled with one spawn option per side, `AsReliableProducer(consumerName)` and `AsReliableConsumer(producerName)`; the actor system creates and manages an internal controller next to each endpoint, which handles sequencing, demand, resends, sessions, and restart deduplication. Controllers are invisible to every actor-management API and impose no naming convention on user actors.
  ...
  - `WithDeliveryConfirmation` on the producer side tells the producer a `DeliveryConfirmed` for every message the consumer confirms, so it can report completion to whoever submitted the work using correlation it recorded at ingress. The notice is best effort within one controller incarnation and repeats when a message is redelivered and confirmed again, so handle it idempotently by `MessageID`; a durable completion record remains the durable queue's job.
  - `WithChunking(maxChunkBytes)` on the producer side splits payloads larger than the setting into parts that each consume one sequence number and reassemble at the consumer controller before `Delivery`, so one large message can no longer exceed the remoting frame cap and kill the connection. The size must be in [`MinChunkSize`, `MaxChunkSize`], and a message must fit in the consumer's flow-control window worth of chunks (a violation fails the flow terminally with the remedy in the error). With a durable queue, chunks are stored atomically through `StoreChunked` under reserved derived identities (`GoAktChunk:<index>/<count>:<businessMessageID>`) so a crash mid-message cannot mix encodings; a resubmission recovers the stored shape even when the re-encoded payload crosses the chunk threshold, and an application `MessageID` must not start with the reserved prefix.
  - The `DurableProducerQueue` contract adds producer-crash survival: `Load` acquires epoch-fenced writership, `Store` is first-write-wins per `MessageID`, `StoreChunked` is first-write-wins for a whole chunked batch keyed by the business `MessageID`, and `Accept` / `Confirm` advance the durable state (`Accept` takes the business `MessageID` and covers every derived chunk of a batch). New sentinels: `ErrQueueFenced`, `ErrQueueConflict`, `ErrReliableStore`, `ErrReliableAccept`, `ErrReliableConfirm`. A durable flow costs roughly two backend round trips per message before the next credit, so backend latency caps per-flow throughput.
  ...
  - Cross-node flows run in one of two modes. In cluster mode both systems join the same GoAkt cluster: endpoint relocation rides the existing replicated actor record, a relocated durable producer reloads its queue under a new epoch that fences the departed writer, queue types must be registered on every eligible node, and the registry needs a replica count of at least 2. In remoting-only mode both systems enable remoting and each side names the peer node's address (`WithRemoteConsumer` on the producer, `WithRemoteProducer` on the consumer): resolution asks the addressed node directly through an internal remoting lookup that validates the endpoint-companion pair on its local tree, a peer address requires remoting (`ErrReliablePeerRemotingRequired`) and rejects clustering (`ErrReliablePeerClusterConflict`), relocation does not exist, and node loss is recovered by restarting the peer at its configured address, after which the ordinary registration resync reconnects the flow. Work-pulling does not support peer addresses. Remoting-only remote placement of a reliable endpoint fails fast with `ErrReliableClusterRequired` (local `Spawn` with `WithHostAndPort`, and the remote spawn handler) instead of spawning a flow that never connects; remoting-only endpoints are always spawned locally on their own node.
  - Terminal failures publish one `ReliableDeliveryFailed` event carrying the endpoint name, controller role, and failing stage while the endpoint stays alive; `ReSpawn` recreates the controller after remediation. Both the role and the stage render by name in formatted output. Transient loss self-heals through the protocol's timers.
  ...
  - **Work-pulling** ([#1300](https://github.com/Tochemey/goakt/issues/1300)). `AsWorkPullingProducer` and `AsWorkPullingWorker` distribute a stream of jobs from one producer across a dynamic set of uniform workers. Each worker reuses the unchanged consumer controller and `Delivery`/`Confirmed` contract; the producer keeps `RequestNext`/`Produced`/`Stored`/`StoredAck`. Guarantees are per-message at-least-once with `MessageID` deduplication and no ordering across workers: per-worker sequencing and demand are isolated, dispatch is round-robin among workers with free demand, and a lost worker requeues its unconfirmed messages under the original `MessageID` for a surviving worker, so workers must process idempotently. Workers register dynamically, and a worker's sub-flow survives its own re-registrations within a producer session; registration fencing accepts only consumer companions whose configuration names the producer, and a worker set spanning nodes requires cluster mode exactly as point-to-point peer resolution does. `WithDeliveryConfirmation` reports each worker confirmation as a `DeliveryConfirmed` carrying the producer-visible `Stored` sequence, repeating when requeued work is confirmed again. `WithChunking` and point-to-point `WithDurableQueue` are rejected; durability uses `WithDurableWorkQueue` / `DurableWorkQueue` with per-`MessageID` `ConfirmMessage` so out-of-order worker completion can leave holes. On reload, accepted unconfirmed jobs re-enter the pending pool and re-dispatch to current workers. Documented on the work-pulling page of the reliable-delivery docs.
