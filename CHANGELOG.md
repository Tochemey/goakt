...
- `Tell` to a remote `*PID` now returns after the message is enqueued for the per-destination coalesced writer rather than after the transport ACK. Transport errors for batched sends are reported asynchronously through the actor system's logger. Callers that need synchronous delivery confirmation should use `Ask` against the remote `*PID`, which is unaffected. Per-call context deadlines set on a remote `Tell` call are not honored by the coalesced path; the writer uses its own transport deadline.
