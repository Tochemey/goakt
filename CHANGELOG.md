...
- **Message handoff buffering during relocation** ([#1255](https://github.com/Tochemey/goakt/issues/1255)). Name-based sends now mask the brief window in which a departed node's actor is being recreated on a survivor. When `SendAsync`/`SendSync` (and their `ReceiveContext` wrappers) route to an actor whose host just left the cluster, the send is buffered (re-resolved and retried with bounded backoff) instead of immediately returning a connection error or a spurious `ErrActorNotFound`, and is delivered once the actor re-registers elsewhere. Each node opens a short, self-expiring handoff window on the departure it observes, so steady-state sends resolve and deliver exactly once with no added latency; a send that outlasts the window returns a retryable error (`ErrActorNotFound` or the new `ErrRelocationInProgress`). A `actorsystem.relocation.buffered.count` metric (tagged with `actor.system`) reports how often sends hit the window. Masking applies to identity-based routing; a send on a stale, pre-departure remote `*PID` still bypasses re-resolution.
