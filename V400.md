...
**Magical Go type serialization:** When you use `WithSerializers(new(MyMessage), remote.NewCBORSerializer())`, the type
is automatically registered in a global type registry. No separate registration step. On serialize, the type name is
embedded in the frame; on deserialize, the registry resolves the type name back to the concrete Go type. Both sender
and receiver must register the same types via `WithSerializers`; proto types use protobuf's own resolution and are
excluded.

- **Type registry:** Types are auto-registered when using `WithSerializers` or `WithClientSerializers` with CBOR
...
- **CBOR:** `remote.NewCBORSerializer()` for arbitrary Go types. Register via `WithSerializers(new(MyMessage), remote.NewCBORSerializer())` — types are **magically** auto-registered; no separate registration step.
