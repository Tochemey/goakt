...
#### 14. `testkit.Probe.SenderAddress()` removed

**Migration:** Use `Sender()` and then `pid.IsRemote()` / `pid.Path()` as needed.

---

#### 15. `ReceiveContext.SenderAddress()` and `ReceiverAddress()` removed
...
#### 18. `actor/messages.go` — native Go system messages
...
#### 19. `actor/messages_serializers.go` — serialization for native types

Provides encoding support for native message types so they can cross process boundaries without protobuf.

---

#### 20. `remote/serializer.go` — pluggable serializer interface
...
#### 21. `remote/proto_serializer.go` — built-in protobuf implementation
...
#### 22. Serializer dispatch — server and client
...
#### 23. `remote/cbor_serializer.go` — CBOR for any Go type
...
#### 24. `Path` interface — location-transparent actor identity
...
#### 25. `remote/actor_state.go` — `ActorState` and `RemoteState`
...
#### 26. `PID.Kind()` — actor kind accessor
...
#### 27. Remote PID — lightweight cross-node handle
...
#### 29. `log/slog.go` and `log/zap.go` — pluggable logger implementations

The log package provides multiple `Logger` implementations:

- **`log.NewZap(level, writers...)`** — Zap-based logger; buffered file output for Info/Warn/Debug; `DefaultLogger` and `DebugLogger` use Zap
- **`log.NewSlog(level, writers...)`** — Standard library `slog`-based logger; low-GC with typed `slog.Attr` for common types
- **`log.DiscardLogger`** — No-op logger for tests

The `Logger` interface is unchanged; choose the implementation that fits your stack (Zap for high throughput, Slog for stdlib-only).

---

### Internal Extractions

#### 30. `internal/commands` package
...
#### 31. `cleanupCluster` singleton kind removal
...
#### 32. `address.Address.String()` — eager caching
...
#### 33. `internal/xsync.List` — deduplication and low-GC redesign
...
#### 34. Scheduler — unified local/remote delivery
...
#### 35. `toReceiveContext` — else-branch elimination
